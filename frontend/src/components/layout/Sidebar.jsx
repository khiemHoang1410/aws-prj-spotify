...
export default function Sidebar() {
  ...
  const { activePlaylistId, activeArtistId, currentView } = useSelector((state) => state.ui); // [S6-003.1]
  ...
  const handlePlaylistClick = (playlist) => {
    ...
    dispatch(setView('playlist-detail'));
  };
    ...
    <nav className="h-full flex flex-col gap-2 p-2">
      ...
      <div className="bg-[#121212] rounded-lg p-2 flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-2">
          ...
          {/* Item cố định: Bài hát đã thích */}
          <div
            className={`flex items-center gap-3 px-2 py-1.5 rounded cursor-pointer mb-1 transition ${
              currentView === 'liked-songs' ? 'bg-white/10' : 'hover:bg-white/5'
            }`}
            onClick={() => dispatch(setView('liked-songs'))}
          >
          ...
          {filter === 'Nghệ sĩ' && (
            isLoadingArtists ? (
              ...
              <div className="flex flex-col gap-0.5">
                {artists.map((artist) => {
                  // [S6-003.2] Active state cho artist row
                  const isActive = activeArtistId === artist.id && currentView === 'artist-profile';
                  return (
                    <div
                      key={artist.id}
                      ...
                      onClick={() => {
                        dispatch(setActiveArtist(artist.id));
                        dispatch(setView('artist-profile'));
                      }}
                    >
                      {isActive && (
                        <div className="absolute left-0 top-0 w-1 h-full bg-green-400 rounded-l" />
                      )}
                      <img
          ...
          {/* PLAYLISTS (chỉ hiện khi không phải tab Nghệ sĩ) */}
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col gap-0.5">
                  {playlists.map((playlist) => {
                    const isActive = activePlaylistId === playlist.id && currentView === 'playlist-detail';
                    return (
                      <div
                        key={playlist.id}
                        className={`relative flex items-center gap-3 px-2 py-1.5 rounded cursor-pointer transition overflow-hidden ${
